...
const dominator = {
  ...
  increment_element : function (el) {
    let id_arr = el.id.split('_')
    let var_arr = el.innerText.split('_')
    let i = Number(id_arr[id_arr.lenth-1])
    i++
    id_arr[id_arr.length-1] = i.toString()
    var_arr[var_arr.length=1] = i.toString()
    el.id = id_arr.join('_')
  },
